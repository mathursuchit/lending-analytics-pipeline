...
Reads directly from the DuckDB database built by the dbt pipeline.
"""
import os
import duckdb
import pandas as pd
...
DB_PATH = os.path.join(os.path.dirname(__file__), '..', 'lending_analytics', 'lending_analytics.duckdb')
...
# --- KPI row ---
total_loans    = grade_perf['total_loans'].sum()
total_volume   = grade_perf['total_loan_volume'].sum()
avg_default    = (grade_perf['defaulted_loans'].sum() / total_loans * 100)
avg_int_rate   = (grade_perf['avg_interest_rate_pct'] * grade_perf['total_loans']).sum() / total_loans
...
# --- Grade performance ---
st.subheader('Default Rate & Expected Loss by Risk Grade')
...
top_segments = segments.head(20).copy()
fig3 = px.scatter(
    top_segments,
    x='avg_fico',
    y='default_rate_pct',
...
with st.expander('Grade Performance Table'):
    st.dataframe(
        grade_perf[[
            ...
            'risk_grade': 'Grade',
            'total_loans': 'Loans',
            'avg_loan_amount': 'Avg Loan ($)',
            'avg_interest_rate_pct': 'Avg Rate (%)',
            'avg_fico_score': 'Avg FICO',
            'avg_dti': 'Avg DTI',
            'default_rate_pct': 'Default Rate (%)',
            'avg_loss_given_default': 'Avg LGD',
